...
class CS_Menu_Item_Custom_Fields_Map {
  ...
  public static function init() {
    ...
    // Disable entirely
    if (!apply_filters("cs_use_custom_fields_map", true)) {
      return;
    }
  ...
  public static function _fields( $id, $item, $depth, $args ) {
    ...
    static::setupEditorScript();

    $field_id =  $id;

    if ( did_action("cs_menu_custom_fields_rendered_{$field_id}") ) {
      return false;
    }
    ...
    foreach ( self::$fields as $_key => $data ) :
      ...
      <?php elseif ( $data['type'] == 'icon' ) :
        ...
        $value = cornerstone('FontAwesome')->resolveFontAlias( $value );
        ...
        <p class="description description-<?php echo $data['size']; ?> <?php echo esc_attr( $class ); ?>">
          <label for="<?php echo esc_attr( $id ); ?>"><?php echo esc_html( $data['label'] ); ?><br>
            <input
              type="text" list="icon-list" name="<?php echo esc_attr( $name ); ?>"
              class="widefat tco-menu-icon-input tco-datalist-input <?php echo esc_attr( $id ); ?>"
    ...
    do_action("cs_menu_custom_fields_rendered_{$field_id}");

  }

  public static function setupEditorScript() {
    static $enqueued;
    ...
    $script = cs_js_asset_get('assets/js/admin/cs-admin-menu-editor');
    wp_register_script( 'cs-admin-menu-editor', $script['url'], $deps, $script['version'] );

    $app = cornerstone('App');
    $app->setArgs([ 'permalinks' => [ '', '' ] ]);
    $app->setupAppConfig('cs-admin-menu-editor', false, false);

    wp_enqueue_script( 'cs-admin-menu-editor' );

    // Styles
    cornerstone('AdminDashboard')->enqueue();
  }
