...
class Cornerstone_Site_Import_Processor {
  ...
  public function importSlider( $data ) {
    ...
    // Rev 6
    if (method_exists('RevSliderSlider', 'isAliasExists') && RevSliderSlider::isAliasExists( $data['name'] ) )
      return false;

    // Rev 7
    $slider = new RevSliderSlider();

    if (method_exists($slider, 'check_alias') && $slider->check_alias( $data['name'] ) )
      return false;
    ...
    try {
      ...
      } else if(class_exists('RevSliderSliderImport')) {
        // Rev7
        require_once( RS_PLUGIN_PATH . 'admin/includes/license.class.php' );
        require_once( RS_PLUGIN_PATH . 'admin/includes/addons.class.php' );
        require_once( RS_PLUGIN_PATH . 'admin/includes/template.class.php' );

        RevSliderGlobals::instance()->add( 'RevSliderAddons', new RevSliderAddons() );

        // Rev 7
        $importer = new RevSliderSliderImport();
        if (method_exists($importer, 'import_slider')) {
          $importer->import_slider( true, $file );
        }
      }
  ...
  public function importImage( $data ) {
    ...
    // Hotlink images that fail to download (usually this means deleted images)
    if ( is_wp_error( $download ) ) {
      $this->registry->set( 'image', $data['id'], $data['url'] );
      $this->message = sprintf( __('Downloading images...', 'cornerstone'), basename( $data['url'] ) );
      return true;
    }
  ...
  public function importTerm( $term ) {
    ...
    if ( $parent != 0 ) {
      ...
      if ( is_null( $parent ) )
        return new WP_Error( 'cornerstone', 'Export file has hierarchical terms out of order. ' . json_encode($term));
  ...
  public function importPostMeta( $data ) {
    ...
    foreach ($data['meta'] as $key => $value) {
      ...
      if ( ! is_null( $maybe_json ) ) {
        // Dont allow empty json
        // Some revisions are empty
        // not sure why but this is the hack
        if (empty($maybe_json)) {
          continue;
        }
      ...
      if ($key === '_cornerstone_data') {
        ...
        //cornerstone('Resolver')->getDocument( $post )->save();
      }
  ...
  public function importOptions( $data ) {
    ...
    if ( isset( $data['cornerstone_font_items'] ) ) {
      ...
      update_option(
        ...
        wp_slash( cs_json_encode( $this->mergeOptionModels(
          get_option('cornerstone_font_items', []),
    ...
    if ( isset( $data['cornerstone_color_items'] ) ) {
      ...
      update_option(
        ...
        wp_slash( cs_json_encode( $this->mergeOptionModels(
          get_option('cornerstone_color_items', []),
    ...
    // Loop through theme options to use
    // update_option
    foreach ($data as $key => $value) {
      $normalized = $this->normalizeContent(maybe_unserialize($value));

      // Font weight migration
      if (strpos($key, "font_weight_selection") !== false) {
        $normalized = apply_filters( 'cs_migrate_font_weight', $normalized );
      }

      // Font family migration
      if (strpos($key, "font_family_selection") !== false) {
        $normalized = apply_filters( 'cs_migrate_font_family', $normalized );
      }

      if ( false === update_option( $key, $normalized ) && get_option($key) != $normalized) { // intentional usage of non-strict equality
    ...
    // Legacy sites never had this
    // false doesn't get updated if it was never set
    // So we change to non false back to false, so the entry is actually in the database
    // https://developer.wordpress.org/reference/functions/update_option/#source
    update_option('x_enable_font_manager', '0', true);
    update_option('x_enable_font_manager', false, true);
...
class Cornerstone_Controller_Design_Cloud extends Cornerstone_Plugin_Component {
  ...
  public function import_site( $data ) {
    ...
    Networking::set_curl_timeout_end();
