...
function x_shortcode_accordion_item( $atts, $content = null ) {
  ...
  $output = "<div {$id} class=\"{$class}\" {$style}>"
            ...
            . "<a {$attr_id_toggle} class=\"x-accordion-toggle{$class_toggle}\" role=\"tab\" data-x-toggle=\"collapse-b\"{$attr_toggleable}{$attr_toggle_group}{$attr_aria_selected}{$attr_aria_expanded}{$attr_aria_controls}>"
              . cs_icon_unicode_get("2b", "x-shortcode-accordion-icon")
              . "<span>{$title}</span>"
            . "</a>"
            . '</div>'
