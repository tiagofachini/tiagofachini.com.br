...
function x_element_render_accordion_item_elements( $data ) {
  ...
  // Content in accordion setup
  $contentAtts = [
    'class'=> 'x-acc-content',
  ];

  // Preview dropzone support
  if (! apply_filters( 'cs_render_looper_is_virtual', false ) && !empty($data['_builder_atts'])) {
    $contentAtts = array_merge($contentAtts, $data['_builder_atts']);
  }
  ...
  if (!is_null($GLOBALS['cs_accordion_faq_items'])) {
    $GLOBALS['cs_accordion_faq_items'][] = [
      'question' => $data['accordion_item_header_content'],
      'answer' => cs_faq_schema_strip($children),
    ];
  }
  ...
  return cs_tag('div', $atts_accordion_item, $custom_atts, [
    ...
    cs_tag('div', $atts_accordion_collapse, [
      cs_tag('div', $contentAtts, $children)
    ])
  ]);
